...
if(isset($_POST['add-post'])){
    ...
    $alt_text = mysqli_real_escape_string($db, $_POST['alt_text']);
 ...
 $check_existing_article = "SELECT * FROM article WHERE slug = '$slug'";
  $check_article = mysqli_query($db, $check_existing_article);
  if(mysqli_num_rows($check_article) > 0){
    ...
    $meta_desc = mysqli_real_escape_string($db, $_POST['meta_desc']);
...
if(isset($_POST['update-post'])){
  ...
  $article = mysqli_real_escape_string($db, $_POST['article']);
  $category = mysqli_real_escape_string($db, $_POST['category']);
  $alt_text = mysqli_real_escape_string($db, $_POST['alt_text']); // Retrieve alt text from form
  $tags_array = array();
  $tags = $_POST['tags'];
  if(!empty($_POST['tags'])){
    foreach ($_POST['tags'] as $tags) {
...
if(mysqli_num_rows($check_article) > 0){
  ...
  if($status){
    ...
    $delete_tags=mysqli_query($db,$sql_delete_tag);
    for ($x = 0; $x < $arrlength; $x++) {
     
        $sql_tags = "INSERT INTO product_tags (post_id, tag_id) VALUES ('$id' , '$tags_array[$x]')";
        $inserttags=mysqli_query($db,$sql_tags);
    }
...
if(isset($_POST['add-tag'])){
  ...
  $check_tags = mysqli_query($db, $check_existing_tags);
...
if(isset($_POST['update-tag'])){
  $id = mysqli_real_escape_string($db,$_POST['tagid']);
  $tag_name = mysqli_real_escape_string($db, $_POST['tag_name']);
  $slug = mysqli_real_escape_string($db, $_POST['slug']);
